...
# Safe message edit
def safe_edit_message_text(bot, text, chat_id, message_id, reply_markup=None, parse_mode="Markdown"):
    try:
        ...
        if "message is not modified" in str(e):
            print("⚠️ Message not modified, skipping edit...")
        else:
            raise e

# Signal message generator
def generate_signal():
    big_small = random.choice(['𝐁𝐈𝐆', '𝐒𝐌𝐀𝐋𝐋'])
    color = random.choice(['🟢', '🔴'])
    number = random.choice(list('𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿'))
    period_id = int(time.time()) % 1000000
    period_display = ''.join(["𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿"[int(d)] for d in str(period_id)])
    msg = f"💢 *𝗛𝗚𝗭𝗬 𝗔𝗨𝗧𝗢 𝗣𝗥𝗘𝗗𝗜𝗖𝗧𝗜𝗢𝗡* 💢\n\n"
    msg += f"⏳ *𝙿𝙴𝚁𝙸𝙾𝙳 𝙸𝙳:* `{period_display}`\n"
    msg += f"🚨 *𝚁𝙴𝚂𝚄𝙻𝚃* --> `{big_small}, {color}, {number}`\n\n"
    msg += f"⭕ *ᗰᑌՏT ᗷᗴ 7-8 ՏTᗴᑭ ᗰᗩIᑎTᗩIᑎ.*"
    return msg
...
def signal_loop():
    while True:
        time.sleep(60)
        if signal_on_channels:
            signal = generate_signal()
            for ch in signal_on_channels:
                try:
                    bot.send_message(f"@{ch}", signal, parse_mode="Markdown")
                    print(f"✅ Signal sent to @{ch}")
...
def handle_callback(call):
    if call.data == "signal_on":
        ...
        markup.row(
            types.InlineKeyboardButton("SIGNAL OFF", callback_data="signal_off"),
        ...
        signal_on_channels.update(added_channels)
        safe_edit_message_text(bot, "✅ *SIGNAL ON COMPLETED*", call.message.chat.id, call.message.message_id, markup)
...
def save_channel_link(message):
    ...
    bot.send_message(message.chat.id, f"✅ Channel [@{username}](https://t.me/{username}) added successfully!", parse_mode="Markdown")
